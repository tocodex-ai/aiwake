...
def read_public_chat_history(hours: int = 24, limit: int = PUBLIC_CHAT_MAX_ITEMS) -> list[dict[str, Any]]:
    ...
    items: list[dict[str, Any]] = []
    items.extend(_read_diary_chat_history(cutoff=cutoff, max_items=max_items))
    items.extend(_read_activity_chat_history(cutoff=cutoff, max_items=max_items))
    for i in range(days):
    ...
    # 同一条消息可能同时来自历史日记和新的 public_chat JSONL，按时间/角色/内容去重。
    deduped: dict[tuple[int, str, str], dict[str, Any]] = {}
    for item in items:
        key = (int(float(item.get("ts") or 0) // 60), str(item.get("role") or ""), str(item.get("content") or "").strip())
        deduped[key] = item
