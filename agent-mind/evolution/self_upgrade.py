...
async def generate_llm_patch(
    ...
    if not raw_output:
        _logger.info("[self_upgrade] LLM returned empty response (transient)")
...
async def generate_and_apply_patch(
    ...
    *,
    ...
    if not patch_result.get("success"):
        ...
        # Transient generation failure — keep status approved for next attempt
        updated["notes"] = list(updated.get("notes") or []) + [
            redact_secrets(f"LLM patch generation failed by {actor}: {patch_result.get('error', 'unknown')[:200]}")
        ]
        updated["patch_generation_event"] = redact_secrets({
            "actor": actor,
            "attempted_at": _utc_now(),
        ...
        append_proposal(CodeUpgradeProposal.from_dict(updated))
        return {"applied": False, "reason": f"LLM patch generation failed: {patch_result.get('error', '')[:200]}"}
    ...
    if applied_count > 0:
        ...
        updated["apply_event"] = redact_secrets({
            ...
            "policy": "LLM 自主生成 search/replace 补丁并受控应用；已通过安全检查，已创建 snapshot 备份。",
        })
    else:
        updated.setdefault("notes", [])
        updated["notes"] = list(updated.get("notes") or []) + [
